...
# sidecar process, VERIFIES zero remain, launches one `yarn start`, then the
# app-level single-instance lock hard-exits any accidental duplicate. Never
# blanket-kills electron.exe/node.exe - always matched by command line, so
# other Electron apps and unrelated node processes are untouched.
