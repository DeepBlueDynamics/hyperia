...
| `tts` | `{recipient?, engine?, elevenlabs?}` | spoken summaries. `recipient` = callsign address (default `"base"`). `engine`: `"auto"` (default — ElevenLabs when a token is present, else Kokoro), `"kokoro"`, or `"elevenlabs"`. `elevenlabs`: `{token, voice?, model?}` — token also read from `ELEVENLABS_API_KEY`; voice defaults to Rachel, model to `eleven_turbo_v2_5`. Kokoro is always the fallback on cloud failure. |
