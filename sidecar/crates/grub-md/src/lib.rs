...
// Public API
//
// grub builds this crate as a PyO3 extension (the binding that previously sat
// here is stripped on vendoring into Hyperia). The core `run_pipeline` is
// unchanged — Hyperia calls it in-process via these thin wrappers. The same
// converter now serves both grub (fetched HTML) and Hyperia web panes
// (the already-rendered DOM, serialized to outerHTML).
// ---------------------------------------------------------------------------

/// Convert HTML to clean reader-mode markdown. `base_url` (may be "") resolves
/// relative links/images to absolute. Picks the main-content node, strips
/// nav/footer/script, and falls back to a full walk if the result is too sparse.
pub fn to_markdown(html: &str, base_url: &str) -> String {
    run_pipeline(html, base_url, true).clean_markdown
}
